...
def _norm_name(value: Any) -> str:
    """Normalize a deal name for cross-board matching (lowercase, trim, collapse spaces)."""
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return ""
    return " ".join(str(value).strip().lower().split())


def won_deals_execution(deals: pd.DataFrame, work_orders: pd.DataFrame) -> dict[str, Any]:
    """Cross-board join: of the deals we WON, how many are actually being
    executed and billed (i.e. have matching Work Orders)?

    Match key: normalized deal name shared between the Deals board (deal_name)
    and the Work Orders board (deal_name). Names are messy, so we match on a
    normalized (lowercased, whitespace-collapsed) form and clearly caveat that
    matching is name-based and therefore approximate.
    """
    if deals.empty or work_orders.empty:
        return {"error": "Both Deals and Work Orders data are required for this join."}
    if "deal_name" not in deals.columns or "deal_name" not in work_orders.columns:
        return {"error": "Deal name column missing on one of the boards; cannot join."}

    won = deals[deals["status"].isin(WON_STATUSES)].copy()
    if won.empty:
        return {"message": "No won deals to analyse."}

    won["_key"] = won["deal_name"].map(_norm_name)
    wo = work_orders.copy()
    wo["_key"] = wo["deal_name"].map(_norm_name)

    won_keys = {k for k in won["_key"].unique() if k}
    wo_by_key = {k for k in wo["_key"].unique() if k}

    matched_keys = won_keys & wo_by_key
    unmatched = won_keys - wo_by_key

    # Distinct won deals (by name) and how many have any work order.
    total_won_names = len(won_keys)
    with_wo = len(matched_keys)

    # For matched work orders, summarize execution + billing health.
    matched_wo = wo[wo["_key"].isin(matched_keys)]
    exec_breakdown = {}
    billing_breakdown = {}
    if "execution_status" in matched_wo.columns:
        exec_breakdown = {
            k: int(v)
            for k, v in matched_wo["execution_status"].fillna("Unknown").value_counts().items()
        }
    if "billing_status" in matched_wo.columns:
        billing_breakdown = {
            k: int(v)
            for k, v in matched_wo["billing_status"].fillna("Unknown").value_counts().items()
        }

    coverage = f"{(with_wo / total_won_names * 100):.0f}%" if total_won_names else "N/A"

    return {
        "won_deal_names": total_won_names,
        "won_with_work_orders": with_wo,
        "won_without_work_orders": len(unmatched),
        "execution_coverage": coverage,
        "matched_work_orders": int(len(matched_wo)),
        "execution_status_of_matched": exec_breakdown,
        "billing_status_of_matched": billing_breakdown,
        "caveat": (
            "Matching is by normalized deal name across the two boards and is "
            "therefore approximate; renamed or differently-spelled deals may not link."
        ),
    }
